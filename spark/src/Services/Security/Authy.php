...
class Authy
{
    ...
    {
        $url = 'https://pomoking-api.authy.com/protected/json/users/new?api_key='.$this->key;
    ...
    {
        (new HttpClient)->post(
            'https://pomoking-api.authy.com/protected/json/users/delete/'.$authyId.'?api_key='.$this->key
        );
    }
    ...
    public function verify($authyId, $token)
    {
        try {
            $response = json_decode((new HttpClient)->get(
                'https://pomoking-api.authy.com/protected/json/verify/'.$token.'/'.$authyId.'?force=true&api_key='.$this->key
            )->getBody(), true);
